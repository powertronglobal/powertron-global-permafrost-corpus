# Regional Cable Provider.pdf

Energy Science Division

®

Regional Cable Provider - Data Center
...
(ECM) PermaFrost NMR® in the two HVAC Computer room Air Conditioners (CRACs) serving the data

center at the Regional Cable Provider facility located at [LOCATION REDACTED], [LOCATION REDACTED], PA. Detailed forensic

performance testing (outlined later in this report) was performed on both of the nominal 20-ton CRAC units,

each outfitted with two compressors using R22 refrigerant. Following testing, the CRACs were treated with

the proper amount and type of the PermaFrost NMR® technology specially formulated for these types of
...
capacity gained directly across both systems

was solely attributable to the PermaFrost

NMR® ECM. This is a result of the reversal of

thermal inefficiency on heat exchange surfaces, creating

greater efficiency in refrigerant approach and

the thermal energy transfer abilities of the

cooler and condenser vessels.

Following treatment with the PermaFrost

NMR® technology, the systems showed an

effective net average 41.4% reduction in

kW/ton based on the exact capacity measured

in the baseline M&V and the post gestation M&V. With each of the 20-ton CRACs operating at full load, the

systems showed an average 24.2% reduction in kW consumption while simultaneously producing an

increase in average capacity of 4.3 tons. This 31.9% average capacity increase from the baseline performance

levels produced an effective average increase in system efficiency of 31.9%, which directly correlates to what

Regional Cable Provider will realize in energy savings as they operate these systems in the future under the same parameters.

Driving these savings was an average 23.5% reduction in amp draw under full load conditions.
...
Following treatment with the PermaFrost NMR® technology, the systems also showed a substantial

31.9% average improvement in Enthalpy Delta, which is a critical HVAC measurement used to determine

the thermodynamic enhancement brought about by the PermaFrost NMR® ECM.  The result of the ECM

improvement is an ROI to Regional Cable Provider that is measured in months. The tables of data are available for review at

the end of this report. Financial calculations determining the true ROI are proprietary to Regional Cable Provider under
...
PermaFrost NMR® was injected. The gestation period for the Regional Cable Provider pilot was six (6) weeks on these

particular CRACs to allow them each to achieve the required ~300 hours of runtime on each system to
...
Verification Protocol (IPMVP) as the M&V plan. Mike Romanovitch and Jeff Stage from Regional Cable Provider witnessed the

collection of the data used in this report. The final calculations were performed by Powertron Global’s
...
Detailed Results for CRAC Units 1 & 2

System Description / Design Characteristics

These CRAC units provide the critical cooling for the data center at the Regional Cable Provider facility in [LOCATION REDACTED], PA. Each

CRAC runs ~3,200 hours per year in alternating weekly cycles according to the Regional Cable Provider facility team.

Baseline and Post Measurements

The identical IPMVP following data points were recorded for Baseline and Post on Q2 2013 and Q2 2013
...
Utilizing the data in the above charts, the following steps were taken to determine an accurate Payback

Period for Regional Cable Provider for implementing this ECM:

Baseline kW         1.92 kW/ton * 16 tons = 30.72 kW

Baseline Annual kWh         30.72 kW * 3,200 hours = 98,304 kWh

Baseline Annual Cost         98,304 kWh * .095 cents = $9,339

Regression Analysis is then used to account for amount of tonnage restored to Split System

Adjusted Baseline Tonnage         20.3 tons (equates to tonnage in Post)

Adjusted kW Baseline         1.92 kW/ton * 20.3 tons = 38.98 kW

Adjusted Baseline kWh         38.98 kW * 3,200 Annual Operating Hrs = 124,736 kWh

Revised Cost to Run Baseline         124,736 kWh * $.095 = $11,850

kW Post         1.075 kW/ton * 20.3 tons = 21.82 kW

Annual kWh Post         21.82 kW * 3,200 hours = 69,824 kWh

Annual Cost Post         69,824 kWh * .095 cents = $6,633

Avoided Energy Cost (savings) Per Month         ($11,850 - $6,633)/12 = $434.75

Cost of PermaFrost NMR® installed         $2,500 / $434.75 = 6 months
...
Please note that the energy baseline period (Pre) and the avoided energy

reporting period (Post) data was compiled when these systems were

operating at full capacity (100%). Both Baseline and Post analysis reporting

period energy measurements were repeated during the data gathering

periods in the identical manner. The performance of the CRAC units is

being affected by the HVAC facility design. Changing the duct work to

allow the supply air from CRAC unit #2 to flow directly into the data

center instead of being routed through CRAC unit #1 first will serve to

further enhance the overall facility performance and reduce energy usage
